...
- **混合解析**：本地无节点或超时时自动回退 WhatsLink，并继续使用 WhatsLink 的预览截图；临时异常会自动重试。
- **轻量缓存**：本地元数据和 WhatsLink 成功结果仅保存在内存中，不缓存视频、截图或其他资源文件；WhatsLink 截图 URL 最多缓存 128 条、15 分钟后过期。
- **无图降级**：资源元数据解析成功但无可用截图时，会明确提示“暂无可用截图”。
